...
from api.deps import get_current_admin, get_current_user, serialize_user
...
def _get_stored_password_hash(document: dict | None) -> str | None:
    if not document:
        return None
...
async def login(payload: LoginRequest) -> TokenResponse:
    db = get_database()
    document = await db.users.find_one({"email": _normalize_email(payload.email)})
    stored_password_hash = _get_stored_password_hash(document)
    if stored_password_hash is None or not verify_password(payload.password, stored_password_hash):
